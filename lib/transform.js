...
exports.setHue = function(colorDict, hue){
	var hslObj = this.rgbToHsl(colorDict);
	hslObj.hue = this.checkRange(hue, 0, 360);
	return this.hslToRgb(hslObj);
};

exports.setSaturation = function(colorDict, sat){
	var hslObj = this.rgbToHsl(colorDict);
	hslObj.saturation = this.checkRange(sat, 0, 100);
	return this.hslToRgb(hslObj);
};

exports.setLightness = function(colorDict, lnes){
	var hslObj = this.rgbToHsl(colorDict);
	hslObj.lightness = this.checkRange(lnes, 0, 100);
	return this.hslToRgb(hslObj);
};
...
exports.rotateHue = function(colorDict, degrees){
	var hslObj = this.rgbToHsl(colorDict);
	hslObj.hue = this.addMod(hslObj.hue, degrees, 360);
	return this.hslToRgb(hslObj);
};

exports.saturate = function(colorDict, percent){
	var hslObj = this.rgbToHsl(colorDict);
	hslObj.saturation = this.checkRange( (hslObj.saturation + percent), 0, 100);
	return this.hslToRgb(hslObj);
};
...
exports.lighten = function(colorDict, percent){
	var hslObj = this.rgbToHsl(colorDict);
	hslObj.lightness = this.checkRange((hslObj.lightness + percent), 0, 100);
	return this.hslToRgb(hslObj);
};
...
exports.invert = function(colorDict) {
	...
	for (var key in colorDict){
		if (key != 'alpha'){
			result[key] = this.addMod(colorDict[key], 128, 255);
		}
	}
	return result;
};
...
exports.setRed = function(colorDict, r){
	return {red: r, green: colorDict.green, blue:colorDict.blue, alpha: colorDict.apha};
};

exports.setGreen = function(colorDict, g){
	return {red: colorDict.red, green: g, blue:colorDict.blue, alpha: colorDict.apha};
};

exports.setBlue = function(colorDict, b){
	return {red: colorDict.red, green: colorDict.green, blue:b, alpha: colorDict.apha};
};
...
exports.addMod = function(left, right, mod){
	return Math.abs((left + right) % mod);
};

exports.minusMod = function(left, right, mod){
	return Math.abs((left - right) % mod);
};

exports.devideMod = function(left, right, mod){
	if(right !== 0) return Math.abs((left / right) % mod);
};

exports.multMod = function(left, right, mod){
	return Math.abs((left + right) % mod);
};
...
exports.add = function(colorDict1, colorDict2) {
	...
	for (var key in colorDict1){
		result.key = this.checkRange(colorDict1.key + colorDict2.key);
	}
	return result;
};
...
exports.divide = function(colorDict1, colorDict2) {
	var result = {};
	for (var key in colorDict1){
		if (colorDict2.key !== 0) result.key = this.checkRange(colorDict1.key / colorDict2.key);
	}
	return result;
};
